...
class MaskedTransformerEncoder(Module):
    ...
    def __init__(self, encoder_layer, num_layers, norm=None, enable_nested_tensor=True, mask_check=True):
        super().__init__()
        torch._C._log_api_usage_once(f"torch.nn.modules.{self.__class__.__name__}") # type: ignore
    ...
    def forward(
            ...
            first_hard_layer: Optional[int] = 1,
            ) -> Tensor:
        ...
        for i, mod in enumerate(self.layers):
            if is_mixed and i % 2 == (1 - first_hard_layer):
                current_mask = None
            else:
                current_mask = mask
