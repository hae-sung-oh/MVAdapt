...
class MVAdapt(nn.Module):
    def __init__(self, config, args=None, device='cuda:0'):
        ...
        # self.gear_length = config.gear_length
        # self.latent_dim = config.mvadapt_latent_dim
        # self.gear_dim = config.mvadapt_gear_dim
        
        # if args is not None:
            # self.latent_dim = args.latent_dim if args.latent_dim is not None else config.mvadapt_latent_dim
            # self.gear_dim = args.gear_dim if args.gear_dim is not None else config.mvadapt_gear_dim
        ...
        self.physics_hidden = nn.Linear(self.latent_dim, self.hidden_size).to(self.device)
    ...
    def forward(self, x, target_point, physics_params, gear_params):
        ...
        z = self.encoder(target_point).unsqueeze(0) + physics_latent
