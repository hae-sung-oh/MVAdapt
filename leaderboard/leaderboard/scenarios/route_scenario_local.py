...
def oneshot_behavior(name, variable_name, behaviour):
    ...
    check_flag = py_trees.blackboard.CheckBlackboardVariable(
        name=variable_name + " Done?", variable_name=variable_name, expected_value=True, clearing_policy=py_trees.common.ClearingPolicy.ON_INITIALISE
    )
...
def convert_json_to_transform(actor_dict):
    ...
    return carla.Transform(
        location=carla.Location(x=float(actor_dict["x"]), y=float(actor_dict["y"]), z=float(actor_dict["z"])), rotation=carla.Rotation(roll=0.0, pitch=0.0, yaw=float(actor_dict["yaw"]))
    )
...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, vehicle_config, debug_mode=0, criteria_enable=True, vehicle_index=0):
        ...
        super(RouteScenario, self).__init__(
            name=config.name, ego_vehicles=[ego_vehicle], config=config, world=world, debug_mode=debug_mode > 1, terminate_on_failure=False, criteria_enable=criteria_enable
        )
    ...
    def _update_route(self, world, config, debug_mode):
        ...
        if debug_mode:
            self._draw_waypoints(world, self.route, vertical_shift=1.0, persistency=50000.0) # type: ignore
    ...
    def _build_scenario_instances(self, world, ego_vehicle, scenario_definitions, scenarios_per_tick=5, timeout=300, debug_mode=False):
        ...
        for scenario_number, definition in enumerate(scenario_definitions):
            ...
            scenario_configuration.ego_vehicles = [ActorConfigurationData("vehicle.lincoln.mkz_2017", ego_vehicle.get_transform(), "hero")]
    ...
    def _initialize_actors(self, config):
        ...
        for _actor in new_actors:
            if _actor.type_id in ["vehicle.carlamotors.carlacola", "vehicle.carlamotors.european_hgv", 
    ...
    def _create_test_criteria(self):
        ...
        blocked_threshold = float(os.environ.get("BLOCKED_THRESHOLD", 180.0))

        blocked_criterion = ActorSpeedAboveThresholdTest(self.ego_vehicles[0], speed_threshold=0.1, below_threshold_max_time=blocked_threshold, terminate_on_failure=True, name="AgentBlockedTest")
