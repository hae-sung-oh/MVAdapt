...
def get_num_jobs(job_name, username, code_root):
    num_running_jobs = int(
        subprocess.check_output(
            f"SQUEUE_FORMAT2='username:{len(username)},name:130' squeue --sort V | " f"grep {username} | grep {job_name} | wc -l",
            shell=True,
        )
        .decode("utf-8")
        .replace("\n", "")
...
def get_carla_command(gpu, num_try, start_port, carla_root):
    command = (
        f"SDL_VIDEODRIVER=offscreen SDL_HINT_CUDA_DEVICE={int(gpu)} {carla_root}/CarlaUE4.sh " f"--carla-world-port={int(gpu)*1000+start_port+num_try*10} -opengl -nosound -carla-streaming-port=0"
    )
    return command


def create_run_eval_bash(bash_save_dir, route_path, scenario_path, gpu, iteration, route, scen, repetition, dataset, checkpoint, agent, start_port, carla_root, code_root, lib_path):
...
def main():
    carla_port = 15000
    code_root = r"/home/ohs-dyros/gitRepo/MVAdapt"
    carla_root = r"/home/ohs-dyros/gitRepo/carla"
    ...
    root_folder = r"/home/ohs-dyros/MVAdataset"  # With ending slash
    ...
    for i in range(len(route_paths)):
            ...
            for name in files:
                if fnmatch.fnmatch(name, route_pattern):
                    ...
                    if root.split("/")[-1].startswith("s"):
                        ...
                        assert os.path.exists(f"{data_routes_dir}/scenarios/eval_scenarios.json"), f'{f"{data_routes_dir}/scenarios/eval_scenarios.json"} does not exist'
        ...
        for ix, route in enumerate(route_files):
            ...
            # job_file = make_jobsub_file(commands=commands, job_number=job_nr, exp_name=exp_name, dataset_name=dataset_name, root_folder=root_folder, node=node, mail=mail)
