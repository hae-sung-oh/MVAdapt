...
//   return;
// }
// // system::system_error e(err, location);
// void throw_exception(system::system_error BOOST_ASIO_SOURCE_LOCATION_ARG) {
//   return;
// }
// }  // namespace boost
class io_context_running {
 public:
  ...
  explicit io_context_running(size_t threads = 2u) : _work_to_do(service) {
    _threads.CreateThreads(threads, [this]() { service.run(); });
  }

  ~io_context_running() { service.stop(); }
...
TEST(streaming, low_level_sending_strings) {
  ...
  carla::streaming::low_level::Server<tcp::Server> srv(io.service,
                                                       TESTING_PORT);
  srv.SetTimeout(1s);
...
TEST(streaming, low_level_tcp_small_message) {
  ...
  srv.Listen(
      [&](std::shared_ptr<tcp::ServerSession> session) {
  ...
  Dispatcher dispatcher{
      make_endpoint<tcp::Client::protocol_type>(srv.GetLocalEndpoint())};
  auto stream = dispatcher.MakeStream();
  auto c = std::make_shared<tcp::Client>(
      io_context, stream.token(), [&](carla::Buffer message) {
        ++message_count;
  ...
  threads.CreateThreads(std::max(2u, std::thread::hardware_concurrency()),
                        [&]() { io_context.run(); });
...
TEST(streaming, multi_stream) {
  ...
  for (auto i = 0u; i < iterations; ++i) {
    std::vector<std::pair<std::atomic_size_t, std::unique_ptr<Client>>> v(
        number_of_clients);
