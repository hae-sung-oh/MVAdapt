...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        # Controllers
        self._turn_controller = PIDController(k_p=self.config.turn_kp, k_i=self.config.turn_ki, k_d=self.config.turn_kd, n=self.config.turn_n)
        self._turn_controller_extrapolation = PIDController(k_p=self.config.turn_kp, k_i=self.config.turn_ki, k_d=self.config.turn_kd, n=self.config.turn_n)
        self._speed_controller = PIDController(k_p=self.config.speed_kp, k_i=self.config.speed_ki, k_d=self.config.speed_kd, n=self.config.speed_n)
        self._speed_controller_extrapolation = PIDController(k_p=self.config.speed_kp, k_i=self.config.speed_ki, k_d=self.config.speed_kd, n=self.config.speed_n)
    ...
    def _init(self, hd_map):
        ...
        self._waypoint_planner = RoutePlanner(self.config.dense_route_planner_min_distance, self.config.dense_route_planner_max_distance)
        self._waypoint_planner.set_route(self.dense_route, True)
        self._waypoint_planner.save()

        self._waypoint_planner_extrapolation = RoutePlanner(self.config.dense_route_planner_min_distance, self.config.dense_route_planner_max_distance)
        ...
        if self.save_path is not None:
            ...
            if self.tp_stats:
                if len(self.tp_sign_agrees_with_angle) > 0:
                    print("Agreement between TP and steering: ", sum(self.tp_sign_agrees_with_angle) / len(self.tp_sign_agrees_with_angle))
    ...
    def _get_brake(self, near_command=None, plant=False):
        ...
        if self.visualize == 1:
            self._world.debug.draw_box(box=ego_bb_global, rotation=ego_bb_global.rotation, thickness=0.1, color=carla.Color(0, 0, 0, 255), life_time=(1.0 / self.config.carla_fps))
        ...
        number_of_future_frames_no_junction = int(self.config.extrapolation_seconds_no_junction * self.config.bicycle_frame_rate)

        # Get future bbs of walkers
        if not plant:
            nearby_walkers = self.forcast_walkers(actors, ego_vehicle_location, ego_vehicle_transform, number_of_future_frames, number_of_future_frames_no_junction)

        # Get future bbs of ego_vehicle
        bounding_boxes_front, bounding_boxes_back, future_steering = self.forcast_ego_agent(ego_vehicle_transform, ego_speed, number_of_future_frames, number_of_future_frames_no_junction)
            ...
            for vehicle in vehicles:
                ...
                if vehicle.get_location().distance(ego_vehicle_location) < self.config.detection_radius:
                    ...
                    if len(self.vehicle_speed_buffer[vehicle.id]["velocity"]) > self.config.stuck_buffer_size:
                        self.vehicle_speed_buffer[vehicle.id]["velocity"] = self.vehicle_speed_buffer[vehicle.id]["velocity"][-self.config.stuck_buffer_size :]
                        self.vehicle_speed_buffer[vehicle.id]["throttle"] = self.vehicle_speed_buffer[vehicle.id]["throttle"][-self.config.stuck_buffer_size :]
                        self.vehicle_speed_buffer[vehicle.id]["brake"] = self.vehicle_speed_buffer[vehicle.id]["brake"][-self.config.stuck_buffer_size :]
                    ...
                    if self.config.model_interactions:
                        ...
                        center_safety_box = traffic_transform.transform(carla.Location(x=traffic_safety_loc[0], y=traffic_safety_loc[1], z=vehicle.bounding_box.location.z))
                        traffic_safety_extent = carla.Vector3D(
                            vehicle.bounding_box.extent.x, vehicle.bounding_box.extent.y * self.config.traffic_safety_box_width_multiplier, vehicle.bounding_box.extent.z
                        )
                    ...
                    if self.visualize == 1 and self.config.model_interactions:
                        self._world.debug.draw_box(box=traffic_safety_box, rotation=traffic_safety_box.rotation, thickness=0.1, color=traffic_safety_color, life_time=(1.0 / self.config.carla_fps))
                    ...
                    for i in range(number_of_future_frames):
                        ...
                        transform = carla.Transform(
                            ...
                            carla.Rotation(pitch=traffic_transform.rotation.pitch, yaw=delta_yaws, roll=traffic_transform.rotation.roll),
                        )

                        # Safety box that models the safety distance that other traffic
                        # participants keep.
                        if self.config.model_interactions:
                            center_safety_box = transform.transform(carla.Location(x=traffic_safety_loc[0], y=traffic_safety_loc[1], z=vehicle.bounding_box.location.z))
                            traffic_safety_box = carla.BoundingBox(center_safety_box, traffic_safety_extent)
                            traffic_safety_box.rotation = transform.rotation

                        if self.config.model_interactions and (
                            self.check_obb_intersection(traffic_safety_box, bounding_boxes_back[i]) or self.check_obb_intersection(traffic_safety_box, bounding_boxes_front[i])
                        ):
                            traffic_safety_color = carla.Color(255, 0, 0, 255)
                            # Set action to break to model interactions
                            action[1] = 0.0
                        ...
                        if self.visualize == 1:
                            self._world.debug.draw_box(box=bounding_box, rotation=bounding_box.rotation, thickness=0.1, color=color, life_time=(1.0 / self.config.carla_fps))
                        veh_future_bbs.append(bounding_box)

                    if (
                        statistics.mean(self.vehicle_speed_buffer[vehicle.id]["velocity"]) < self.config.stuck_vel_threshold
                        and statistics.mean(self.vehicle_speed_buffer[vehicle.id]["throttle"]) > self.config.stuck_throttle_threshold
        ...
        if not plant:
            for i, elem in enumerate(zip(bounding_boxes_front, bounding_boxes_back)):
                ...
                if self.visualize == 1:
                    self._world.debug.draw_box(box=bounding_box, rotation=bounding_box.rotation, thickness=0.1, color=color, life_time=(1.0 / self.config.carla_fps))
                    self._world.debug.draw_box(box=bounding_box_back, rotation=bounding_box.rotation, thickness=0.1, color=color2, life_time=(1.0 / self.config.carla_fps))
        ...
        bremsweg = (((ego_speed * 3.6) / 10.0) ** 2 / 2.0) + self.config.safety_box_safety_margin

        index_future_orientation = int((bremsweg / self.target_speed) * self.config.bicycle_frame_rate)
        ...
        for o in range(min(index_drive_margin + index_future_orientation, number_of_future_frames)):
            ...
            next_loc_brake, next_yaw_brake, next_speed_brake = self.ego_model.forward(next_loc_brake, next_yaw_brake, next_speed_brake, action_brake)
            index = o
            if o >= len(future_steering):
                index = len(future_steering) - 1
            action_brake[0] = future_steering[index]

        center_safety_box = ego_vehicle_transform.transform(carla.Location(x=next_loc_brake[0], y=next_loc_brake[1], z=self._vehicle.bounding_box.location.z))
        ...
        bounding_box.rotation.yaw = float(t_u.normalize_angle_degree(np.rad2deg(next_yaw_brake) + bounding_box.rotation.yaw))
        ...
        # Stop sign detection
        # -----------------------------------------------------------
        self.stop_sign_hazard = self.ego_agent_affected_by_stop_sign(ego_vehicle_transform, ego_vehicle_location, actors, ego_speed, bounding_box)
    ...
    def forcast_ego_agent(self, vehicle_transform, speed, number_of_future_frames, number_of_future_frames_no_junction):
        ...
        for i in range(number_of_future_frames):
            ...
            bounding_box.rotation = carla.Rotation(pitch=vehicle_transform.rotation.pitch, yaw=delta_yaws_no_brake, roll=vehicle_transform.rotation.roll)
            ...
            bounding_box_back.rotation = carla.Rotation(pitch=vehicle_transform.rotation.pitch, yaw=delta_yaws_no_brake, roll=vehicle_transform.rotation.roll)
    ...
    def forcast_walkers(self, actors, vehicle_location, vehicle_transform, number_of_future_frames, number_of_future_frames_no_junction):
        ...
        for walker in walkers:
            if walker.get_location().distance(vehicle_location) < self.config.detection_radius:
                ...
                relative_pos = t_u.get_relative_transform(np.array(vehicle_transform.get_matrix()), np.array(walker_transform.get_matrix()))
    ...
    def ego_agent_affected_by_red_light(self, vehicle_transform, detection_box):
        ...
        for light, center, waypoints in self.list_traffic_lights:
            ...
            for wp in waypoints:
                ...
                length_bounding_box = carla.Vector3D((wp.lane_width / 2.0) * 0.9, light.trigger_volume.extent.y, light.trigger_volume.extent.z)

                bounding_box = carla.BoundingBox(wp.transform.location, length_bounding_box)

                gloabl_rot = light.get_transform().rotation
                bounding_box.rotation = carla.Rotation(
                    pitch=light.trigger_volume.rotation.pitch + gloabl_rot.pitch, yaw=light.trigger_volume.rotation.yaw + gloabl_rot.yaw, roll=light.trigger_volume.rotation.roll + gloabl_rot.roll
                )
                ...
                affects_ego = False
                if self.check_obb_intersection(detection_box, bounding_box) or self.check_obb_intersection(vehicle_bb, bounding_box):
                    affects_ego = True
                ...
                if self.visualize == 1:
                    ...
                    self._world.debug.draw_point(wp.transform.location + carla.Location(z=light.trigger_volume.location.z), size=0.1, color=color, life_time=0.01)
    ...
    def ego_agent_affected_by_stop_sign(self, vehicle_transform, vehicle_location, actors, speed, safety_box):
        ...
        for stop_sign in stop_signs:
            ...
            if self.visualize:
                self._world.debug.draw_box(box=bounding_box_stop_sign, rotation=bounding_box_stop_sign.rotation, thickness=0.1, color=color, life_time=(1.0 / self.config.carla_fps))
    ...
    def check_obb_intersection(self, obb1, obb2):
        ...
        return not (
            self.get_separating_plane(r_pos, obb1.rotation.get_forward_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, obb1.rotation.get_right_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, obb1.rotation.get_up_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, obb2.rotation.get_forward_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, obb2.rotation.get_right_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, obb2.rotation.get_up_vector(), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_forward_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_right_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_forward_vector(), obb2.rotation.get_up_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_forward_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_right_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_right_vector(), obb2.rotation.get_up_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_forward_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_right_vector()), obb1, obb2)
            or self.get_separating_plane(r_pos, self.cross_product(obb1.rotation.get_up_vector(), obb2.rotation.get_up_vector()), obb1, obb2)
        )
