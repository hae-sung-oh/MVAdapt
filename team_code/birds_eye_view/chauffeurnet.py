...
class ObsManager(ObsManagerBase):
    ...
    def get_observation(self, close_traffic_lights=None):
        ...
        def is_within_distance(w):
            c_distance = abs(ev_loc.x - w.location.x) < self._distance_threshold and abs(ev_loc.y - w.location.y) < self._distance_threshold and abs(ev_loc.z - w.location.z) < 8.0
            # Cheap trick to remove the ego bounding box
            c_ev = abs(ev_loc.x - w.location.x) < self.config.ego_extent_y and abs(ev_loc.y - w.location.y) < self.config.ego_extent_y
        ...
        for vehicle in vehicles:
            ...
            bounding_box = carla.BoundingBox(traffic_transform.location + vehicle.bounding_box.location, vehicle.bounding_box.extent)
        ...
        vehicle_masks, walker_masks, tl_green_masks, tl_yellow_masks, tl_red_masks, stop_masks = self._get_history_masks(m_warp)
